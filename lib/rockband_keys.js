...
function RockBandKeyboard(device) {
    ...
    this.specialButtons = { Minus: 1, Plus: 2, Menu: 16 };
    _.each(this.specialButtons, function(v, k) { this.specialButtonData[k] = false }, this);
    ...
    function processData(data) {
        ...
        _.each(this.buttons, function(button, name) {
            ...
            if (this.buttonData[name] != pressed) {
                 ...
                 * @param {Object} data             Event data.
                 * @param {Boolean} data.pressed    Button pressed or not.
                 * @param {String} data.name        Which button was pressed ("1", "A", "B", "2", "Plus", "Minus", "Menu").
                 */
        ...
        var specialButtonsPressed = data[1];
        _.each(this.specialButtons, function(button, name) {
            var pressed = (specialButtonsPressed & button) != 0;
            if (this.specialButtonData[name] != pressed) {
                this.emit("button", {name: name, pressed: pressed });
                this.specialButtonData[name] = pressed;
            }
        }, this);
